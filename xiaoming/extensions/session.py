...
# -*- coding: utf-8 -*-
"""
    xiaoming.extensions.session
    ~~~~~~~~~~~~~~~~~~~~

    add session module to tornado
...
class Session(object):
    ...
    def __init__(self, get_secure_cookie, set_secure_cookie, name='_session', expires_days=None):
        """init Session from secure cookie"""
...
class RedisSession(object):
    ...
    def access(self, remote_ip):
        """loggin access ip and time in redis"""
        access_info = {
            'remote_ip': remote_ip,
            'time': '%.6f' % time.time()
        }
        self._store.set_session(self._sid, 'last_access', pickle.dumps(access_info))
    ...
    def save(self):
        if self._dirty:
            self._store.set_ssion(self._sid, self._data, 'data', self._expiry)
            self._dirty = False